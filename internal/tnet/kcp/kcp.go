...
func smuxConf(cfg *conf.KCP) *smux.Config {
	...
	sconf.Version = 2
	sconf.KeepAliveInterval = time.Duration(cfg.SmuxKeepAliveInterval) * time.Millisecond
	sconf.KeepAliveTimeout = time.Duration(cfg.SmuxKeepAliveTimeout) * time.Millisecond
